...
AetherArena is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, persistent memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.
...
│   ├── packages/
│   │   └── harness/           # aether-harness package (import: aether.*)
│   │       ├── pyproject.toml
│   │       └── aether/
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/aether/` never imports from `app.*`
...
- **Harness** (`packages/harness/aether/`): Publishable agent framework package (`aether-harness`). Import prefix: `aether.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config — everything needed to build and run agents.
...
**Dependency rule**: App imports aether-arena, but aether never imports app. This boundary is enforced by `tests/test_harness_boundary.py` which runs in CI.

**Import conventions**:
```python
# Harness internal
from aether.agents import make_lead_agent
...
**Lead Agent** (`packages/harness/aether/agents/lead_agent/agent.py`):
- Entry point: `make_lead_agent(config: RunnableConfig)` registered in `langgraph.json`
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions

**ThreadState** (`packages/harness/aether/agents/thread_state.py`):
...
Middlewares execute in strict order in `packages/harness/aether/agents/lead_agent/agent.py`:

1. **ThreadDataMiddleware** - Creates per-thread directories (`backend/.aether-arena/threads/{thread_id}/user-data/{workspace,uploads,outputs}`)
...
2. `AETHER_ARENA_CONFIG_PATH` environment variable
...
Configuration priority:
1. Explicit `config_path` argument
2. `AETHER_ARENA_EXTENSIONS_CONFIG_PATH` environment variable
...
### Sandbox System (`packages/harness/aether/sandbox/`)

**Interface**: Abstract `Sandbox` with `execute_command`, `read_file`, `write_file`, `list_dir`
**Provider Pattern**: `SandboxProvider` with `acquire`, `get`, `release` lifecycle
**Implementations**:
- `LocalSandboxProvider` - Singleton local filesystem execution with path mappings
- `AioSandboxProvider` (`packages/harness/aether/community/`) - Docker-based isolation

**Virtual Path System**:
- Agent sees: `/mnt/user-data/{workspace,uploads,outputs}`, `/mnt/skills`
- Physical: `backend/.aether-arena/threads/{thread_id}/user-data/...`, `aether-arena/skills/`
- Translation: `replace_virtual_path()` / `replace_virtual_paths_in_command()`
- Detection: `is_local_sandbox()` checks `sandbox_id == "local"`

**Sandbox Tools** (in `packages/harness/aether/sandbox/tools.py`):
...
### Subagent System (`packages/harness/aether/subagents/`)
...
### Tool System (`packages/harness/aether/tools/`)
...
### Skills System (`packages/harness/aether/skills/`)

- **Location**: `aether-arena/skills/{public,custom}/`
- **Format**: Directory with `SKILL.md` (YAML frontmatter: name, description, license, allowed-tools)
- **Loading**: `load_skills()` recursively scans `skills/{public,custom}` for `SKILL.md`, parses metadata, and reads enabled state from extensions_config.json
- **Injection**: Enabled skills listed in agent system prompt with container paths
- **Installation**: `POST /api/skills/install` extracts .skill ZIP archive to custom/ directory

### Model Factory (`packages/harness/aether/models/factory.py`)
...
Bridges external messaging platforms (Feishu, Slack, Telegram) to the AetherArena agent via the LangGraph Server.
...
### Memory System (`packages/harness/aether/agents/memory/`)
...
**Data Structure** (stored in `backend/.aether-arena/memory.json`):
...
### Reflection System (`packages/harness/aether/reflection/`)
...
Both can be modified at runtime via Gateway API endpoints or `AetherArenaClient` methods.

### Embedded Client (`packages/harness/aether/client.py`)

`AetherArenaClient` provides direct in-process access to all AetherArena capabilities without HTTP services. All return types align with the Gateway API response schemas, so consumer code works identically in HTTP and embedded modes.

**Architecture**: Imports the same `aether-arena` modules that LangGraph Server and Gateway API use. Shares the same config files and data directories. No FastAPI dependency.
