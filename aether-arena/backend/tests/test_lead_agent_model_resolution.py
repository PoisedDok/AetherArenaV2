...
from aether.agents.lead_agent import agent as lead_agent_module
from aether.config.app_config import AppConfig
...
def test_make_lead_agent_disables_thinking_when_model_does_not_support_it(monkeypatch):
    ...
    import aether.tools as tools_module
    ...
    def _fake_create_chat_model(*, name, thinking_enabled, reasoning_effort=None, provider_api_key=None, **kwargs):
        captured["name"] = name
...
def test_make_lead_agent_bootstrap_includes_setup_agent(monkeypatch):
    """Bootstrap runs must bind setup_agent; configurable is set by the web client."""
    app_config = _make_app_config([_make_model("safe-model", supports_thinking=False)])
    ...
    monkeypatch.setattr(lead_agent_module, "get_app_config", lambda: app_config)
    monkeypatch.setattr(tools_module, "get_available_tools", lambda **kwargs: [])
    monkeypatch.setattr(lead_agent_module, "_build_middlewares", lambda *a, **k: [])
