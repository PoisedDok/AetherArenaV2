...
import aether.config.app_config as app_config_module
from aether.agents.checkpointer import get_checkpointer, reset_checkpointer
from aether.config.checkpointer_config import (
    CheckpointerConfig,
...
# ---------------------------------------------------------------------------
# AetherArenaClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """AetherArenaClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            ...
            patch("aether.client.AetherArenaClient._get_tools", return_value=[]),
        ):
            client = AetherArenaClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = AetherArenaClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
