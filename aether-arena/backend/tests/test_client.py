...
class TestConfigQueries:
    def test_list_models(self, client):
        ...
        assert result["models"][0]["provider_use"] == "langchain_openai:ChatOpenAI"
        assert result["models"][0]["endpoint_url"] == "https://api.example.com/v1"
    ...
    def test_list_skills(self, client):
        ...
        with patch("aether.skills.loader.load_skills", return_value=[skill]) as mock_load:
    ...
    def test_list_skills_enabled_only(self, client):
        with patch("aether.skills.loader.load_skills", return_value=[]) as mock_load:
            client.list_skills(enabled_only=True)
            mock_load.assert_called_once_with(enabled_only=True)

    def test_get_memory(self, client):
        memory = {"version": "1.0", "facts": []}
        with patch("aether.agents.memory.updater.get_memory_data", return_value=memory) as mock_mem:
...
class TestEnsureAgent:
    def test_creates_agent(self, client):
        ...
        with (
            patch("aether.client.create_chat_model"),
    ...
    def test_uses_default_checkpointer_when_available(self, client):
        ...
        with (
            patch("aether.client.create_chat_model"),
            patch("aether.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("aether.client._build_middlewares", return_value=[]),
            patch("aether.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("aether.agents.checkpointer.get_checkpointer", return_value=mock_checkpointer),
        ):
            client._ensure_agent(config)
...
class TestMcpConfig:
    def test_get_mcp_config(self, client):
        ...
        with patch("aether.client.get_extensions_config", return_value=ext_config):
    ...
    def test_update_mcp_config(self, client):
        ...
        try:
            ...
            with (
                patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("aether.client.get_extensions_config", return_value=current_config),
                patch("aether.client.reload_extensions_config", return_value=reloaded_config),
            ):
...
class TestSkillsManagement:
    ...
    def test_get_skill_found(self, client):
        ...
        with patch("aether.skills.loader.load_skills", return_value=[skill]):
            result = client.get_skill("test-skill")
        assert result is not None
        assert result["name"] == "test-skill"

    def test_get_skill_not_found(self, client):
        with patch("aether.skills.loader.load_skills", return_value=[]):
    ...
    def test_update_skill(self, client):
        ...
        try:
            ...
            with (
                patch("aether.skills.loader.load_skills", side_effect=[[skill], [updated_skill]]),
                patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("aether.client.get_extensions_config", return_value=ext_config),
                patch("aether.client.reload_extensions_config"),
            ):
    ...
    def test_install_skill(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aether.skills.loader.get_skills_root_path", return_value=skills_root),
                patch("aether.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "my-skill")),
            ):
...
class TestMemoryManagement:
    def test_reload_memory(self, client):
        ...
        with patch("aether.agents.memory.updater.reload_memory_data", return_value=data):
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("aether.config.memory_config.get_memory_config", return_value=config):
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("aether.config.memory_config.get_memory_config", return_value=config),
            patch("aether.agents.memory.updater.get_memory_data", return_value=data),
        ):
...
class TestArtifacts:
    def test_get_artifact(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aether.client.get_paths", return_value=mock_paths):
...
class TestScenarioConfigManagement:
    ...
    def test_model_and_skill_discovery(self, client):
        ...
        model_cfg.use = "langchain_openai:ChatOpenAI"
        model_cfg.supports_thinking = False
        model_cfg.supports_reasoning_effort = False
        model_cfg.model_dump.return_value = {}
        client._app_config.get_model_config.return_value = model_cfg
    ...
    def test_mcp_update_then_skill_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aether.client.get_extensions_config", return_value=current_config),
                patch("aether.client.reload_extensions_config", return_value=reloaded_config),
            ):
            ...
            with (
                patch("aether.skills.loader.load_skills", side_effect=[[skill], [toggled]]),
                patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aether.client.get_extensions_config", return_value=ext_config),
                patch("aether.client.reload_extensions_config"),
            ):
...
class TestScenarioMemoryWorkflow:
    ...
    def test_memory_full_lifecycle(self, client):
        ...
        with patch("aether.agents.memory.updater.get_memory_data", return_value=initial_data):
            mem = client.get_memory()
        assert len(mem["facts"]) == 1

        with patch("aether.agents.memory.updater.reload_memory_data", return_value=updated_data):
            refreshed = client.reload_memory()
        assert len(refreshed["facts"]) == 2

        with (
            patch("aether.config.memory_config.get_memory_config", return_value=config),
            patch("aether.agents.memory.updater.get_memory_data", return_value=updated_data),
        ):
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aether.skills.loader.get_skills_root_path", return_value=skills_root),
                patch("aether.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "my-analyzer")),
            ):
            ...
            with patch("aether.skills.loader.load_skills", return_value=[installed_skill]):
                skills_result = client.list_skills()
            ...
            with (
                patch("aether.skills.loader.load_skills", side_effect=[[installed_skill], [disabled_skill]]),
                patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aether.client.get_extensions_config", return_value=ext_config),
                patch("aether.client.reload_extensions_config"),
            ):
...
class TestGatewayConformance:
    ...
    def test_list_models(self, mock_app_config):
        ...
        model.supports_thinking = False
        model.supports_reasoning_effort = False
        model.model_dump.return_value = {"base_url": "https://api.openai.com/v1"}
        ...
        assert parsed.models[0].provider_use == "langchain_openai:ChatOpenAI"
        assert parsed.models[0].endpoint_url == "https://api.openai.com/v1"
    ...
    def test_get_model(self, mock_app_config):
        ...
        assert parsed.provider_use == "langchain_openai:ChatOpenAI"
    ...
    def test_install_skill(self, client, tmp_path):
        ...
        custom_dir.mkdir()
        with patch("aether.skills.loader.get_skills_root_path", return_value=tmp_path):
            result = client.install_skill(archive)
    ...
    def test_update_mcp_config(self, client, tmp_path):
        ...
        with (
            patch("aether.client.get_extensions_config", return_value=ext_config),
            patch("aether.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
            patch("aether.client.reload_extensions_config", return_value=ext_config),
        ):
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("aether.config.memory_config.get_memory_config", return_value=mem_cfg):
            result = client.get_memory_config()
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("aether.config.memory_config.get_memory_config", return_value=mem_cfg),
            patch("aether.agents.memory.updater.get_memory_data", return_value=memory_data),
        ):
            result = client.get_memory_status()
