...
if [ -n "$AETHER_ARENA_CONFIG_PATH" ] && [ -f "$AETHER_ARENA_CONFIG_PATH" ]; then
    CONFIG="$AETHER_ARENA_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to aether-arena.*',
        'replacements': [
            ('src.community.', 'aether-arena.community.'),
            ('src.sandbox.', 'aether-arena.sandbox.'),
            ('src.models.', 'aether-arena.models.'),
            ('src.tools.', 'aether-arena.tools.'),
        ],
    },
