---
name: claude-to-aether-arena
description: "Interact with AetherArena v2 (this agent harness) via its HTTP API. Use when the user wants to send messages, start threads, check health, list models/skills/agents, manage memory, upload files, or delegate research-style tasks. Also use when the user mentions aether-arena, deer flow, or the local agent stack (historical names)."
---

# AetherArena v2 HTTP bridge (`claude-to-aether-arena`)

Communicate with a running instance via its HTTP API. AetherArena v2 is an agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

The stack exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$AETHER_ARENA_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$AETHER_ARENA_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
| `AETHER_ARENA_GATEWAY_URL`  | `${AETHER_ARENA_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `AETHER_ARENA_LANGGRAPH_URL`| `${AETHER_ARENA_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
AETHER_ARENA_URL="${AETHER_ARENA_URL:-http://localhost:2026}"
AETHER_ARENA_GATEWAY_URL="${AETHER_ARENA_GATEWAY_URL:-$AETHER_ARENA_URL}"
AETHER_ARENA_LANGGRAPH_URL="${AETHER_ARENA_LANGGRAPH_URL:-$AETHER_ARENA_URL/api/langgraph}"
```
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$AETHER_ARENA_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$AETHER_ARENA_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$AETHER_ARENA_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$AETHER_ARENA_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$AETHER_ARENA_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$AETHER_ARENA_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$AETHER_ARENA_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-aether-arena/scripts/chat.sh "Your question here"
```
...
- If health check fails, the backend is not running. Tell the user to start Docker/`make` per their setup.
