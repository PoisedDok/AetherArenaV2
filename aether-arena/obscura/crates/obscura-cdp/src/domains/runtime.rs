    ...
    match method {
        ...
        "evaluate" => {
            ...
            let return_by_value = params
                ...
                .unwrap_or(false);
            // awaitPromise is accepted for CDP compatibility but synchronous evaluation
            // is used because QuickJS resolves pending microtasks during evaluate().
            // For async patterns requiring true Promise awaiting, use Runtime.callFunctionOn
            // with awaitPromise: true (already fully implemented).
