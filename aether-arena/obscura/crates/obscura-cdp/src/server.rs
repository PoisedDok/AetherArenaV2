...
pub async fn start_with_options(
    ...
    // Bind on 0.0.0.0 so the CDP server is reachable from other containers on
    // the same Docker network, not just from the local loopback interface.
    // In production the port is not published through nginx — it stays internal.
    let addr = SocketAddr::from(([0, 0, 0, 0], port));
    let listener = TcpListener::bind(&addr).await?;

    info!("Obscura CDP server listening on ws://0.0.0.0:{}", port);
    info!(
