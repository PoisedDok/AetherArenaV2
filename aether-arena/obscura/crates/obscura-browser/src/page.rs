...
impl Page {
    ...
    async fn execute_scripts(&mut self) {
        ...
        if let Some(js) = &mut self.js {
            // 5 s deadline — React / Vue SPAs schedule their reconciler work via
            // MessageChannel / setTimeout callbacks that fire in the event loop.
            // The original 500 ms window was too short for large webpack bundles;
            // increasing to 5 s lets the scheduler complete its first render pass.
    ...
    async fn navigate_single(
        ...
        if matches!(
            ...
            // 15 s max — SPA chunk fetches can be slow on first load; give scripts enough
            // time to request all their async chunks before declaring network idle.
